...
import { GlobalViewCfg } from "../../../../../Loader/MoyeView/GlobalViewCfg";
...
export class LobbyViewCfg extends GlobalViewCfg {
    doShowAnimation(view: AMoyeView, task: Task<any>): void {
        ...
        tween(view.node).to(0.2, { scale: v3(1, 1, 1) }, { easing: 'sineIn' }).call(() => {
            task.setResult();
        }).start();
    }
    ...
    doHideAnimation(view: AMoyeView, task: Task<any>): void {
        view.bringToFront();
        tween(view.node).to(0.2, { scale: v3(0, 0, 0) }, { easing: 'sineOut' }).call(() => {
            task.setResult();
        }).start();
    }
}